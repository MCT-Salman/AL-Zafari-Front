...
export default function ConstantValue() {
  ...
  const constantValueApiAdapter = useMemo(() => ({
    ...
    getItems: async () => {
      ...
      if (selectedMaterialId === "all") {
        ...
        // Fetch all values from all materials
        ...
        for (const material of materials) {
          ...
          if (response.success && response.data) {
            ...
            allValues.push(...valuesWithMaterialName);
    ...
    getItemById: (...args) => constantApi.getConstantValueById(...args),
  ...
  const handleExport = () => {
    ...
    const exportData = filteredValues.map(value => ({
      ...
      material_name: selectedMaterialId === "all"

        ? materials.find(m => m.material_id === value.material_id)?.material_name || ''
    ...
    exportToExcel(exportData, selectedMaterialId === "all" ? 'قيم_جميع_المواد' : `قيم_${selectedMaterialName}`);
  ...
  const handleSaveValue = async (idOrValueData, valueData) => {
    ...
    // For create mode, use formData directly
    ...
    if (!isEditMode) {
      ...
      const dataToSend = {
        ...
        label: formData.label || `${value} ${unit}`.trim(),
    ...
    // For edit mode, use actualValueData
  ...
  let filteredValues = constantValues.filter(
    ...
    (value) => {
      ...
      const matchesSearch =
        ...
        value.material_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const selectedMaterialName = selectedMaterialId === "all" ? "جميع المواد" : materials.find(m => m.material_id === parseInt(selectedMaterialId))?.material_name || "";
        ...
        <Card className="p-6">
          ...
          <div className="-my-4">
            ...
            <SearchInput
              ...
              placeholder={`ابحث عن قيمة (المادة: ${selectedMaterialName} أو القيمة أو العنوان أو الوحدة أو الملاحظات)`}
                ...
                <Table onSort={handleSort}>
                  ...
                  <TableHeader>
                    ...
                    <TableRow>
                      ...
                      <TableHead sortable sortKey="material_name">المادة</TableHead>
                    ...
                    {paginatedValues.map((value) => (
                      ...
                      <TableRow key={value.constant_value_id || value.id}>
                        ...
                        <TableCell className="font-medium">
                          ...
                          {selectedMaterialId === "all" ? value.material_name : (materials.find(m => m.material_id === parseInt(selectedMaterialId))?.material_name || "")}
        ...
        fields={
          ...
          modalState.mode === 'view'
            ...
            ? [
              ...
              { key: 'material_name', label: 'المادة', formatValue: (key, value) => value || materials.find(m => m.material_id === selectedItem?.material_id)?.material_name || '-' },

              { key: 'value', label: 'القيمة', formatValue: (key, value) => value?.toString() || '-' },
